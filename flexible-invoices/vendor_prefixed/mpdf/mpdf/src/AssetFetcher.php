...
use WPDeskFIVendor\Mpdf\File\LocalContentLoaderInterface;
use WPDeskFIVendor\Mpdf\File\StreamWrapperChecker;
use WPDeskFIVendor\Mpdf\Http\ClientInterface;
use WPDeskFIVendor\Mpdf\Log\Context as LogContext;
use WPDeskFIVendor\Mpdf\PsrHttpMessageShim\Request;
use WPDeskFIVendor\Mpdf\PsrLogAwareTrait\PsrLogAwareTrait;
use WPDeskFIVendor\Psr\Log\LoggerInterface;
class AssetFetcher implements \WPDeskFIVendor\Psr\Log\LoggerAwareInterface
{
    ...
    public function fetchDataFromPath($path, $originalSrc = null)
    {
        ...
        if ($wrapperChecker->hasBlacklistedStreamWrapper($path)) {
            throw new \WPDeskFIVendor\Mpdf\Exception\AssetFetchingException('File contains an invalid stream. Only ' . implode(', ', $wrapperChecker->getWhitelistedStreamWrappers()) . ' streams are allowed.');
        }
    ...
    public function fetchRemoteContent($path)
    {
        ...
        try {
            ...
            if ($this->mpdf->debug) {
                throw new \WPDeskFIVendor\Mpdf\MpdfException($message, 0, \E_ERROR, null, null, $e);
            }
