...
class Currency
{
    ...
    private function set_currencies_locale_from_woocommerce()
    {
        if (function_exists('WC')) {
            ...
            foreach ($locales as $locale) {
                ...
                $this->currencies[$currency_slug] = ['decimal_separator' => $decimal_separator, 'thousand_separator' => $thousand_separator, 'currency_position' => $currency_position, 'currency_symbol' => Configs\Currency::get_currency_symbol($currency_slug), 'currency' => $currency_slug];
            }
        }
    }
    /**
    ...
    private function set_currencies_locale_from_wordpress()
    {
        ...
        foreach ($currency_options as $currency) {
            ...
            $this->currencies[$currency['currency']]['currency_symbol'] = Configs\Currency::get_currency_symbol($currency['currency']);
    ...
    {
        $defaults = ['decimal_separator' => '.', 'thousand_separator' => '', 'currency_position' => 'right_space', 'currency_symbol' => Configs\Currency::get_currency_symbol($this->currency_slug), 'currency' => $this->currency_slug];
    ...
    {
        $sign = '';
        $amount = CalculateTotals::price_to_float($amount);
        if ($amount < 0) {
            $sign = '-';
        }
        $currency_option = $this->get_currency_settings();
        $ret = number_format(abs($amount), 2, $currency_option['decimal_separator'], $currency_option['thousand_separator']);
        $symbol = Configs\Currency::get_currency_symbol($currency_option['currency']);
    ...
    {
        $value = CalculateTotals::price_to_float($value);
