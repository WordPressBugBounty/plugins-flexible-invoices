...
final class MossSettingsFields implements SubTabInterface
{
    ...
    private function get_doc_link(): string
    {
        if (InvoicesIntegration::is_super()) {
            ...
            return sprintf('<a href="%1$s&utm_content=oss" target="_blank" style="color: #8f0350; font-weight: 700;">%2$s</a>', Plugin::upgrade_to_pro_url(), esc_html__('Upgrade to PRO and enable options below →', 'flexible-invoices'));
        }
    }
    /**
     * @return array|Field[]
    ...
    {
        $moss = 'MOSS';
        return [(new SubStartField())->set_label(esc_html__('OSS', 'flexible-invoices'))->set_name('moss'), (new Header())->set_label(esc_html__('OSS Handling', 'flexible-invoices'))->set_description($this->get_doc_link()), (new DisableFieldProAdapter('woocommerce_eu_vat_vies_validate', (new FICheckboxField())->set_name('')->set_label(esc_html__('VIES Validation', 'flexible-invoices'))->set_sublabel(esc_html__('Enable', 'flexible-invoices'))->add_class('hs-beacon-search woocommerce_eu_vat_vies_validate')->set_attribute('data-beacon_search', $moss)))->get_field(), (new DisableFieldProAdapter('woocommerce_eu_vat_failure_handling', (new SelectField())->set_name('')->set_label(esc_html__('Failed Validation Handling', 'flexible-invoices'))->set_options(['reject' => esc_html__('Reject the order and show the customer an error message.', 'flexible-invoices'), 'accept_with_vat' => esc_html__('Accept the order, but do not remove VAT.', 'flexible-invoices'), 'accept_without_vat' => esc_html__('Accept the order and remove VAT.', 'flexible-invoices')])->add_class('vies-validation-fields hs-beacon-search')->set_attribute('data-beacon_search', $moss)))->get_field(), (new DisableFieldProAdapter('woocommerce_moss_tax_classes', (new WooSelect())->set_name('')->set_label(esc_html__('Tax class for OSS', 'flexible-invoices'))->set_description(esc_html__('Select the tax classes that the plugin shall use to handling the OSS.', 'flexible-invoices'))->set_options($this->get_woocommerce_tax_classes())->set_attribute('multiple', 'multiple')->add_class('select2 vies-validation-fields hs-beacon-search')->set_attribute('data-beacon_search', $moss)))->get_field(), (new DisableFieldProAdapter('woocommerce_moss_validate_ip', (new FICheckboxField())->set_name('')->set_label(esc_html__('Collect and Validate Evidence', 'flexible-invoices'))->set_sublabel(esc_html__('Enable', 'flexible-invoices'))->set_description(esc_html__('Option validates the customer IP address against their billing address, and prompts the customer to self-declare their address if they do not match.', 'flexible-invoices'))->add_class('vies-validation-fields hs-beacon-search')->set_attribute('data-beacon_search', $moss)))->get_field(), (new DisableFieldProAdapter('woocommerce_reverse_charge_description', (new InputTextField())->set_name('')->set_label(esc_html__('Reverse charge description', 'flexible-invoices'))->set_default_value(esc_html__('Reverse charge', 'flexible-invoices'))->add_class('vies-validation-fields hs-beacon-search')->set_attribute('data-beacon_search', $moss)))->get_field(), (new DisableFieldProAdapter('woocommerce_vat_moss_description', (new InputTextField())->set_name('')->set_label(esc_html__('VAT OSS rate description', 'flexible-invoices'))->add_class('vies-validation-fields hs-beacon-search')->set_attribute('data-beacon_search', $moss)))->get_field(), (new SubEndField())->set_label('')];
    }
    /**
     * @return string
     */
